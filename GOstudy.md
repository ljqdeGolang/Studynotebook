...
- **基础**
  ...
  打印：print 没有换行符，println有换行符，printf格式化输出（用占位符）：%3d,格式化三位输出，不足补空格（int类型），%.3f，保留三位小数（浮点型）%t:bool类型   %c: byte类型   %s: string类型   %p: 内存地址占位符   %T打印对应的数据类型    %b:打印二进制。
  ...
  如果函数的参数为不定参func add(a ...int)，传递参数方式a[0:]...   用len获得不定参的长度。
  ...
  工程管理：src：源码文件夹  pkg: “.a”的归档文件。bin：go install安装形成的可执行文件

  数组：指定数组下标进行初始化值，var arry [10]int{6:10}，第七个元素为10。
  ...
  os包是文件创建的包，`\`在go语言中被认为是转义字符，所以反斜杠用`\\`,也可以用正斜杠`/`来作为文件目录。创建文件例子：
...
3、当用range遍历切片时，改变其值，也能改变切片的值吗？

4、下面的条件语句中的中断命令能否跳出循环？
